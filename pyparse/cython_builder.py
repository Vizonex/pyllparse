"""Used to build apis and more from llparse"""


from pyparse.frontend import IFrontendResult
    ...
    def get_user_callbacks(self):
        ...
        for s in self.info.resumptionTargets:
            for slot in s.ref.buildSlots():
                if isinstance(slot.node.ref, Invoke) and isinstance(slot.node.ref.code.ref, Match):
                ...
                for _slot in slot.node.ref.getSlots():
                    if isinstance(_slot.node.ref, Invoke) and isinstance(_slot.node.ref.code.ref, Match):
...
class ApiCompiler(MainCompiler):
    ...
    def __init__(self, new_preifx:str, info: IFrontendResult,header_name:Optional[str] = None) -> None:
    ...
    def build_C(self):
        ...
        for data in sorted(self.data_cb):
            writer.putline(f"int {data}({self.new_prefix}_t* s, const char* p, const char* endp) " + "{")
            with writer.indent():
                writer.putline("int err;")
                writer.putline(f"CALLBACK_MAYBE(s, {data.removeprefix(prefix).strip('_')}, s, p, endp - p);")
        ...
        for data in sorted(self.cb):
            ...
            with writer.indent():
                writer.putline("int err;")
                writer.putline(f"CALLBACK_MAYBE(s, {data.removeprefix(prefix).strip('_')}, s);")
                writer.putline("return err;")
            writer.putline("}")
            writer.endline()
            writer.endline()

        writer.putline(f"int {self.new_prefix}_execute({self.new_prefix}_t* parser, const char* data, size_t len) " + "{")
        with writer.indent():
        ...
        writer.putline(f"void {self.new_prefix}_settings_init({self.new_prefix}_settings_t* settings)" + " {")
    ...
    def build_H(self,headerguard:Optional[str] = None):
        ...
        writer.putline(F"#ifndef {headerguard}_API_H_")
        writer.putline(F"#define {headerguard}_API_H_")
        writer.endline()
        writer.putline_with_format("typedef %s_t %s_t;",self.info.prefix,self.new_prefix)
        writer.putline(f"typedef struct {self.new_prefix}_settings_s {self.new_prefix}_settings_t;")
        writer.endline()
        writer.putline(f"typedef int (*{self.new_prefix}_data_cb)({self.new_prefix}_t*, const char *at, size_t length);")
        ...
        with writer.indent():
            for data in self.data_cb:
                writer.putline(f"{self.new_prefix}_data_cb {data.removeprefix(self.info.prefix).strip('_')};")
            for data in self.cb:
                writer.putline(f"{self.new_prefix}_cb {data.removeprefix(self.info.prefix).strip('_')};")
        writer.putline("};")
        writer.endline()
        writer.putline(f"int {self.new_prefix}_execute({self.new_prefix}_t* parser, const char* data, size_t len);")
        writer.putline(f"void {self.new_prefix}_init({self.new_prefix}_t* parser,")
    ...
    def build_pxd(self):
        ...
        with writer.indent():
            ...
            writer.endline()
            writer.putline(f"ctypedef int (*{self.new_prefix}_data_cb)({self.new_prefix}_t*, const char *at, size_t length)")
            writer.putline(f"ctypedef int (*{self.new_prefix}_cb)({self.new_prefix}_t*)")
            writer.endline()
            ...
            writer.endline()
            writer.putline(f"int {self.new_prefix}_execute({self.new_prefix}_t* parser, const char* data, size_t len)")
            writer.putline(f"void {self.new_prefix}_init({self.new_prefix}_t* parser, const {self.new_prefix}_settings_t* settings)")
        writer.endline()
        return writer.code
