from .pyfront.front import Identifier, IWrap, SpanField, Span
...
from .tire import (
    Tire, TireNode, TireEmpty, TireSequence, TireSingle
)
...
class Frontend:
    def __init__(self,prefix:str,implementation:IImplementation = IImplementation() ,options:dict[Literal["maxTableElemWidth","minTableSize"],int] = dict()) -> None:
        self.prefix = prefix 
        ...
        self.prefix = prefix
        self.options:dict[Literal["maxTableElemWidth","minTableSize"],int] = {"maxTableElemWidth":options.get("maxTableElemWidth",DEFAULT_MAX_TABLE_WIDTH),
            "minTableSize":options.get("minTableSize",DEFAULT_MIN_TABLE_SIZE)}
        
        if 0 > self.options["maxTableElemWidth"]:
            raise AssertionError("Invalid `options.maxTableElemWidth`, must be positive")
        

    # TODO (Vizonex) : Test Run Frontend Compiler starting tomorrow...
    def compile(self,root:source.code.Node,properties:list[source.Property] = []):
        ...
        for index , concurrent in enumerate(sourceSpans.concurrency):
            ...
            span = SpanField(index,[self.translateCode(c.callback) for c in concurrent])
    ...
    def translateMatch(self,node:source.code.Match):
        trie = Tire(node.name)
    ...
    def translate(self,node:source.code.Node):
        ...
        ID = lambda : self.Id.id(node.name)

        nodeImpl = self.implementation.node

        if isinstance(node,source.code.Error):
            result = nodeImpl.Error(
                _frontend.node.Error(ID(),node.code,node.reason))
            
        elif isinstance(node,source.code.Pause):
            result = nodeImpl.Pause(
                _frontend.node.Error(ID(),node.code,node.reason))
        ...
        if isinstance(result,list):
            ...
            if not otherwise:
                ...
                for child in result:
                    if not child.ref.otherwise:
                        child.ref.setOtherwise(self.translate(otherwise.node),
                            otherwise.noAdvance)
            ...
            if otherwise:
                single.ref.setOtherwise(self.translate(otherwise.node),
                otherwise.noAdvance)
            
            else:
                
                if not isinstance(node,source.code.Error):
                    raise AssertionError(f'Node "{node.name}" has no `.otherwise()')
            
            if isinstance(single.ref, _frontend.node.Invoke):
                for edge in node:
                    single.ref.addEdge(ord(edge.key) if isinstance(edge.key,str) else edge.key, self.translate(edge.node))
            else:
                assert len(list(node)) == 0
            
            return single 
    
    def maybeTableLookup(self,node:source.code.Match,trie:TireSingle,
        children:MatchChildren):
        ...
        for child in trie.children:
            if isinstance(child.node,TireEmpty):
                bailout = False
                continue
                
        
            empty : TireEmpty = child.node
            ...
            # TODO (Vizonex) see if breaking or continue block after out is breakout has been determined is good ot not...
            bailout = True 
            continue
        ...
        if (targets.size >= (1 << self.options["maxTableElemWidth"])):
            return 
        ...
        for _next , target in targets.items():
            table.ref.addEdge(_frontend.node.ITableEdge(
                keys=target.keys,
    ...
    # TODO (Vizonex) fix all "tires" to "tries" :/

    def translateSequence(self,node:source.code.Match, trie:TireSequence,
        children:MatchChildren) -> IWrap[_frontend.node.Match]:
        ...
        value = trie.child.value if isinstance(trie.child,TireEmpty) else None 
    ...
    def translateTrie(self,node:source.code.Match,trie:TireNode,
        children:MatchChildren):
        
        if isinstance(trie,TireEmpty):
            assert self.Map.get(node)
            
            return self.translate(trie.node)
        elif isinstance(trie,TireSingle):
            return self.translateSingle(node, trie, children)
        elif isinstance(trie,TireSequence):
            return self.translateSequence(node,trie,children)
        else:
            raise TypeError("Unknown trie node")
    
    def translateSingle(self,node:source.code.Match,trie:TireSingle,
        children:MatchChildren):
        ...
        for child in trie.children:
            ...
            single.ref.addEdge(
                ...
                noAdvance= child.noAdvance,
                node = childNode,
                value = child.node.value if isinstance(child.node,TireEmpty) else None
            )

        otherwise = trie.otherwise
        if otherwise:
            single.ref.setOtherwise(
                self.translateTrie(node,otherwise,children),
                True,
                otherwise.value
            )
        return single 
    ...
    def translateCode(self,code:source.code.Code):
        ...
        if isinstance(code,source.code.IsEqual):
            ...
            _frontend.code.IMulAddOptions(code.options.base,code.options.max,code.options.signed))
            ...
            res = codeImpl.Update(_frontend.code.Update(prefixed,code.field,code.value))
    ...
    def translateSpanCode(self,code:source.code.Span) -> IWrap[_frontend.code.Span]:
        return self.translateCode(code) 

    def translateTransform(self,transform:Optional[source.code.Transform]
    ) -> IWrap[Union[_frontend.transform.Transform,_frontend.transform.ID,
        _frontend.transform.ToLower,_frontend.transform.ToLowerUnsafe]]:
