...
NOTE that I am using python since I translated llparse to python 
to help me come up with ideas and concepts since I'm not at professional with typescript but 
doing this first is to help me develop this idea from typescript. 
...
class Settings:
    ...
    def move_cb(self,callback:Union[str,source._Match, source.Node,source.Invoke], new_location:str, code:Optional[str] = None):
        ...
        name = callback.name if isinstance(callback,(source.Code,source.Node)) else callback 
    ...
    def add_external_execute(self,postfix_as_struct:bool=False):
        ...
        if self.postfix:
            self.external_execute_func += f"int {self.postfix}_execute({self.postfix if postfix_as_struct else self.prefix}_t* parser, const char* data, size_t len) " + "{"
            self.external_execute_func += f"\nreturn {self.prefix}_execute(parser, data, data + len);\n" + "}"
    ...
    def build_functions(self,out:list[str]):
        ...
        for cb, is_span in self.callbacks.items():
            out.append(f"int {cb}({self.prefix}_t * s, const char* p, const char* endp) " + "{")
            out.append("  int err;")
            if is_span:
    ...
    def buildMacros(self,out:list[str],callback:str = CALLBACK_MAYBE,span_callback:str = SPAN_CALLBACK_MAYBE):
        """Builds Global Macro Callbacks """
        # Inspired by llhttp 
        out.append(f"#define {callback}(PARSER, NAME)                                          \\")
        out.append("do {                                                                        \\")
        out.append(f"  const {self.prefix}{self.name}_t* settings;                               \\")
        out.append(f"  settings = (const {self.prefix}{self.name}_t*) (PARSER)->{self.name};      \\")
        out.append("  if (settings == NULL || settings->NAME == NULL) {                         \\")
        out.append("    err = 0;                                                                \\")
        out.append("    break;                                                                  \\")
        out.append("  }                                                                         \\")
        out.append("  err = settings->NAME((PARSER));                                           \\")
        out.append("} while (0)")
        out.append("")
        
        out.append(f"#define {span_callback}(PARSER, NAME, START, LEN)                         \\")
        out.append("  do {                                                                        \\")
        out.append(f"  const {self.prefix}{self.name}_t* settings;                               \\")
        out.append(f"  settings = (const {self.prefix}{self.name}_t*) (PARSER)->{self.name};      \\")
        out.append("    if (settings == NULL || settings->NAME == NULL) {                         \\")
        ...
        out.append("      break;                                                                  \\")
        out.append("    }                                                                         \\")
        out.append("    err = settings->NAME((PARSER), (START), (LEN));                           \\")
        # TODO Vizonex -- maybe allow for custom error functions to be raised right here 
    ...
    def build_H(self):
        ...
        out.append(f"typedef int (*{fix}_data_cb)({fix}_t*, const char *at, size_t length);")
