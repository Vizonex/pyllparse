    ...
    def is_valid(args,kwargs):
        if kwargs.get("field"):
            ...
            if re.search(r"[//\s\\]+",field):
                raise TypeError(f"Can\'t access internal field because the field: \"{field}\" conatins invalid characters")
    ...
    def __init__(self, code:Code, IInvokeMap:dict[int,Node]) -> None:
        ...
        for numKey, targetNode in IInvokeMap.items():
            if isinstance(numKey, int) and targetNode is None:
                raise TypeError("Invoke's map keys must be integers and values must not be left blank!")
...
class Edge:
    def __init__(self,node:Node,noAdvance:bool,key:Optional[Union[int,str]],value:Optional[int]) -> None:
        self.node = node 
        ...
        if isinstance(node,Invoke):
            ...
            if not isinstance(value,int):
                if node.code.signature != "match":
                    raise TypeError(f"Invalid invoke code signature : {node.code.signature} is not match")
                
            elif node.code.signature != 'value':
                raise TypeError(f"Invalid invoke code signature : {node.code.signature} is not value")
            ...
            if not isinstance(node,Node):
                raise TypeError(f"Attempted to pass value to non-Invoke node as :{type(node)}")
...
to matched character with optional attached value (See `.select()`)"""
    ...
    **must not** be an `Invoke` node with a callback expecting the value.
    ...
    - `next`  Target node to be executed on success."""
        ...
        if len(value) != 1:
            raise AssertionError(f".peek() accepts only singular character keys " + f"perhaps you meant to say : {value.split()}" if isinstance(str,value) else "")
    ...
    def select(self,keyOrDict:Union[int,str,dict[str,int]],valueOrNext:Optional[Union[int,Node]] = None,next:Optional[Node] = None):
...
- `next` Convenience param. Same as calling `.otherwise(...)`"""
        ...
        if not next:
            raise AssertionError("Invalid `next` of argument .select()")

        key = toBuffer(keyOrDict)
        edge = Edge(next,False,key,valueOrNext)
        self.addEdge(edge)
        return self 
