...
class Property:
    def __init__(self,ty:str,name:str) -> None:
        if ty not in PropertyTypes:
            raise Exception(f"Can't use property : {ty}  Because it is not a valid property type in ['i8' , 'i16' , 'i32' , 'i64' , 'ptr']")
        if any([(True if n in name else False) for n in ["\\\\","/"," "]]):
            raise Exception(f"Flag/Pointer Name:{name} cannot have spaces or other strange characters in C...")
        self.name = name 
        self.ty = ty 
...
class Creator:
    ...
    def mulAdd(self,field:str, base:int , max:Optional[int] = None , signed:bool = False ):
...
class Builder:
    ...
    def invoke(self,fn:code.Code,Map:Union[dict[int,str],code.Node,None] = None,otherwise:Optional[code.Node] = None):
    ...
    def property(self,ty:Literal['i8','i16','i32','i64','ptr'] , name:str):
        ...
        if self.privProperties.get(name):
            raise Exception(f"Duplicate property with name:{name}")
        
        prop = Property(ty,name)
        self.privProperties[name] = prop 
        return 
