...
from .frontend import _frontend, IFrontendResult, IWrap, WrappedNode, WrappedCode
...
class Transform:
    def __init__(self,ref:_frontend.transform.Transform) -> None:
        self.ref = ref 
...
class External(Code):
    def __init__(self, ref: _frontend.code.External):
        self.ref = ref 
        super().__init__(ref)
...
class Consume(Node):
    ...
    def doBuild(self, out: list[str]):
        ...
        if ty == "i64":
            ...
            raise Exception(f"Unsupported type {ty} of field {self.ref.field} for consume node")

        out.append(f'avail = {ctx.endPosArg()} - {ctx.posArg()};')
        out.append(f'{fieldTy} need;')
        out.append('  p += need;')
...
class Error(Node):
    ...
    def doBuild(self, out: list[str]):
        self.storeError(out)
        out.append(f"{self.compilation.currentField()} = (void*)(intptr_t) {STATE_ERROR};")
...
class Sequence(Node):
    ...
    def doBuild(self, out: list[str]):
        ...
        out.append(f'match_seq = {matchSequence}({ctx.stateArg()}, ' +
            f'{ctx.posArg()},' +
        ...
        self.tailTo(tmp,
            noAdvance=False,
            node=self.ref.Edge.node,
            value=self.ref.Edge.value
            )
...
class TableLookup(Node):
    ...
    def buildSSE(self,out:list[str]):
        ...
        for off in range(0,len(ranges),SSE_RANGES_LEN):
            ...
            out.append(f'  /* Find first character that does not match \'ranges\' */')
...
class Compilation:
    ...
    def buildBlobs(self,out:list[str]):
        ...
        for blob in self.blobs.values():
            ...
            # so to cut out some redundancy 
            # I'll join these two parts into one - Vizonex 
            if blob.alignment:
    ...
    def buildCode(self,code:Code) -> str:
        if self.codeMap.get(code.ref.name):
            if self.codeMap[code.ref.name].__dict__ != code.__dict__:
                raise AssertionError(f'Code name conflict for "{code.ref.name}"   {self.codeMap.get(code.ref.name).__dict__} != {code.__dict__}')
    ...
    def unwrapCode(self,code:IWrap[_frontend.code.Code],allow_continue:bool=False):
        ...
        if isinstance(ref,_frontend.code.And):
            ...
            # TODO Vizonex Fix Spans since Span Doesn't have an id with it...
    ...
    def unwrapNode(self,node:IWrap[_frontend.node.Node]):
        ...
        if isinstance(ref,_frontend.node.Consume):
            ...
            raise Exception(f'refrence "{ref}" is an Invalid Code Type , TypeName:"{ref.__class__.__name__}"')
    ...
    def errorPosField(self):
        return self.stateField(f'error_pos')
