...
from .C_compiler import CCompiler, Compilation, ICompilerOptions, STATE_PREFIX
...
def smaller_test():
    ...
    complete = p.invoke(p.code.match("On_Complete"),
        {0:simple},p.error(1,"On_Complete FAILED!"))

    simple.peek("\n",complete).skipTo(simple)
    
    front = Frontend("http_parser")
    f = front.compile(simple,properties=p.properties)
    comp = Compilation("http_parser",f.properties,f.resumptionTargets,ICompilerOptions(None,None))
...
def test1():
    ...
    method.select({
        ...
        'TRACE': 7, 'PATCH': 8
    }, onMethod).otherwise(p.error(5, 'Expected method'))
    ...
    front = Frontend("http_parser")
    f = front.compile(method,properties=p.properties,)
    ...
    comp = Compilation("http_parser",p.properties(),f.resumptionTargets,ICompilerOptions())
    root = comp.unwrapNode(f.root)
    root.build(comp)
