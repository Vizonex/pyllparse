from .pybuilder.main_code import Node, Span , SpanEnd , SpanStart, Reachability
...
class SpanAllocator:
        ...
        for node , spans in info.active.items():
            for edge in node.getAllEdges():
                ...
                for subSpan in edgeSpans:
                    if not subSpan in spans:
                        raise AssertionError(f'Unmatched span end for "{subSpan.callback.name}"'/
                        f'at "{edge.node.name}", coming from "{node.name}"')
                
                if isinstance(edge.node,SpanEnd):
                    span = _id(edge.node)
                    if not span in spans:
                        raise AssertionError(f'Unmatched span end for "{span.callback.name}"')
        ...
        for node in queue:
            ...
            for span in active:
                ...
                for edge in node.getAllEdges():
                    ...
                    # TODO Vizonex Make a DEADLOOP Exception Not Just as a Joke but as a Pure Example of the dangers of a DeadLoop
                    # NO LOOPS!!!

                    if isinstance(edgeNode,SpanStart):
                        if _id(edgeNode) == span:
                            raise DeadLoop(f"Detected loop in span {span.callback.name} at \"{node.name}\"")
    ...
    def color(self,spans:list[Span] ,overlapDict:SpanOverlap):
        ...
        # print(concurrency)
        # return ISpanAllocatorResult(colors=MP,concurrency=concurrency, max = _max)

        # TODO (Vizonex) Make Testfunction for spans to make sure it's working correctly...
